...
import type { Idea } from "./ideasData";
...
export function ChatScreen({ idea, onBack }: { idea: Idea; onBack: () => void }) {
  const founder = `${idea.title.split(" ")[0]} Lead`;
  const [messages, setMessages] = useState<Msg[]>([
    { id: "1", from: "them", text: `Yo! Stoked you're into ${idea.title} 🙌` },
  ...
  return (
    <div className="flex h-dvh flex-col">
      <header className="flex items-center gap-3 border-b border-border bg-card/95 px-4 pb-3 pt-12 backdrop-blur-md">
        <button onClick={onBack} className="flex h-9 w-9 items-center justify-center rounded-full active:scale-90" aria-label="Back">
        ...
        >
          {initialsOf(idea.title)}
        </div>
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-bold">{founder}</div>
          <div className="flex items-center gap-1.5">
            <span className="truncate text-xs text-muted-foreground">{idea.title}</span>
            <span
      ...
      <div
        ...
        style={
          isMe
            ...
            : { backgroundColor: "var(--cream, #F4EFE2)", color: "#1A3C2E", borderBottomLeftRadius: 6 }
        }
      >
        {children}
      </div>
